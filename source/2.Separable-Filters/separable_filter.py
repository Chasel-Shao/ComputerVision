...
filename = '../../resource/lena_gray_512.png'
raw_image = np.array(Image.open(filename).convert('L')).astype(float)

size = 7
mean_kernel = np.ones(shape=(size, size), dtype=float) / (size * size)
separable_kernel_1 = np.ones(shape=(1, size), dtype=float) / size
separable_kernel_2 = np.ones(shape=(size, 1), dtype=float) / size
